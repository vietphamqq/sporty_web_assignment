...
def pytest_configure(config):
    ...
    if hasattr(config.option, 'screenshot_on_failure') and config.getoption("--screenshot-on-failure"):
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if (hasattr(config.option, 'allure_report') and config.getoption("--allure-report") and
        hasattr(config.option, 'open_allure') and config.getoption("--open-allure")):
                ...
                def open_allure_report():
                    try:
                        ...
                        process = subprocess.Popen(
                            ["allure", "serve", allure_dir, "--port", "0"],  # port 0 = auto-assign
                        ...
                        print("⚠️  Allure CLI not found. The allure-pytest Python package is installed,")
